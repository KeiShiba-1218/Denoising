...
def plot_losses(opt_paths, a):
    for opt_path in opt_paths:
        ...
        for QF in [10,20,30,40,50,60,70,80,90]:
            test_log = pd.read_csv(os.path.join(log_dir, f'test_losses_{model_name}_{QF}.csv'))
            ...
            if QF==10: points = [QF, point_loss_G, point_psnr, point_ssim]

            ax1.plot(test_log['step'], test_log['loss_G'], label=f'{QF}', alpha=1.0)
            ax1.scatter(*point_loss_G, color='red', s=10)
            ax2.plot(test_log['step'], test_log['psnr'], label=f'{QF}', alpha=1.0)
            ax2.scatter(*point_psnr[:2], color='red', s=10)
            ax3.plot(test_log['step'], test_log['ssim'], label=f'{QF}', alpha=1.0)
            ax3.scatter(*point_ssim, color='red', s=10)
        ax1.set_title(f'Loss_G {model_name}, QF{points[0]}: {points[1]}')
        ax2.set_title(f'PSNR {model_name}, QF{points[0]}: {points[2]}')
        ax3.set_title(f'SSIM {model_name}, QF{points[0]}: {points[3]}')
        ax1.legend()
        ax2.legend()
        ax3.legend()
